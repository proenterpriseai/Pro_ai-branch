...
> ✅ 2026-06-28: **v=20260628a~l 전부 배포 LIVE (최신 git tag `v20260628l`)**. (+k=헤더 네비 글자 15→16px·간격 gap-10→12·지원하기 버튼 셰브론 제거+글자 13→14px. +l=인재양성 로드맵 헤드라인 쉼표 제거·문의하기 뱃지 삭제+위로·서브 끝 em-dash 제거. +m=4축브랜드 헤드라인 "상담 전," 제거→"이미 신뢰가 만들어집니다". +n=STEP1 카드 클릭→프로사업단총괄 오버레이 신인/저차월 교육(#edu-horiz-scroll)로 이동(스크롤러=#pro-intro-overlay 자체, ceo-overlay--active 부착 시 MutationObserver가 initEduScroll 실행)·why-pro "같은 시간," 쉼표 제거·stories "만듭니다." 마침표 제거. +o=STEP1→교육 이동 시 히어로(사무실사진) 플리커 제거: setTimeout450→동기즉시+rAF(init이 스크롤 리셋 안 하고 섹션 세로위치 CSS고정이라 기다릴 필요 없음). +p=히어로 제목 그라데이션(from-white to-white/60)→순백색 text-white. +q=전문역량특화 패널 태그 #데이터영업→#DB영업·#전환율극대화→#소득극대화(q=실적극대화→r=소득극대화)·#심리화법→#전문가양성. +s=히어로 제목 md:text-7xl→6xl(72→60px). +t=히어로 서브타이틀 font-light→normal(흰색인데 얇아서 회색처럼 보이던 것 또렷하게).) 오늘 한 일: 인재양성 로드맵 아이콘·글자 키움 / CEO 인사말 뱃지 삭제(위치고정 visibility:hidden) / why-pro "같은 시간 다른 결과" 블루 프레임(#solutions 동일) + 우측 카드 순서 스왑(보장분석 AI 최상단, 본문만 스왑) / "프로솔루션"→"프로 솔루션" / 승격 구조 파란 배경 단계별 순환(7s, 고정 하이라이트 제거) / **STEP1 궤도 세련화**(빛 스윕 아크+글래스 위성+중앙 PRO 문구+코어 네이비 톤다운+위성 블루 모노톤 통일) / 문구 다수("PRO AI 통합 시스템"·"프로 솔루션 · 7대 AI 시스템"·"통합 금융계산기"·"AI 시스템 체험하기"·stories 캡션 "AI 시스템 · …") / 시스템현황 클릭 글로우 cyan #06b6d4→블루 #3b82f6 / STEP2·3 그래픽 배경 #05030F 통일 / STEP2 리스트 애니 시작잔상 수정(기본 흐림) / CEO 서명 이미지 라이브 복구(assets/images/ceo-signature.png). **전부 DOM 실측 검증·콘솔0.** ⚠️ preview_eval은 백그라운드 탭(docHidden)이라 CSS 애니 frozen → screenshot 타임아웃·동적 색 샘플 dim. 검증 시 `getAnimations().currentTime` 수동주입 또는 base/규칙 직접 읽기로.
