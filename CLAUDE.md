...
- **v=20260627i** (최신, index.html + `assets/video/intro-tunnel.mp4` 추가) — **메인 히어로 "터널 인트로" 빌드 (🔴 `FEATURE_INTRO_TUNNEL=false` 기본=라이브 0 영향).** 첫 진입 시 SF 터널 영상(Pixabay 무료 상업·콘크리트 터널, ~13MB) 3초 재생 → 페이드아웃하며 히어로 "보험을 넘어 성장의 정점으로" 등장. `<body>` 직후 **독립 블록**(자체 `<style>`+`<div#intro-tunnel>`+`<script>`). **블루 틴트**(`.intro-tint` `mix-blend-mode:color #1d49c8` opacity .55) + 비네트. **세션당 1회**(`sessionStorage._intro_seen`) + **Skip 버튼** + reduced-motion 시 스킵. **3초만 재생**(timeupdate≥3→finish) + 3.5s 안전장치 + finish 시 `vid.removeAttribute('src')`로 13MB 끝까지 안 받음. muted/playsinline autoplay. **옵트인 검증**: URL `?intro=1`(미리보기·seen 무시, 가장 간편) 또는 `sessionStorage._flag_intro_tunnel='true'`. ⚠️ 13MB는 무겁→추후 Canva 3초 재export로 경량 파일 교체 권장. 승인 후 Flag true.
- **v=20260627h** (index.html 단일) — 챗 인디케이터·타이핑 클로드化(대표님 요청). **A-1**: `appendTyping` 점 3개 제거 → **PRO AI 아바타 `animate-pulse` 깜빡임** + 오른쪽 문구. 문구를 **시간 기반("Ns · 상태")**으로 — 매초 경과초↑ + 임계값(0/4/10/18s)으로 단계 전환(빠른 자동회전 폐기, "진짜 작업 중" 체감). PDF는 `noRotate`라 기존 %구동 유지. **B-1**: `_solStreamReply` 부드러운 타이핑 — SSE는 `target`에 누적, 화면은 `shown`이 한 자씩 따라잡으며 노출(`revealTimer` 18ms, 멀리 뒤처지면 `ceil(remaining/18)`로 빨리 따라잡고 가까우면 1자씩). 뭉텅이 청크 → 부드러운 타이핑. `finalize()`가 따라잡기 완료+streamDone 시 rich 렌더+CTA. 에러/빈응답 폴백 유지. 코칭·보험금·완판 공통.
