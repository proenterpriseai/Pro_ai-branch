...
> ✅ 2026-06-28: **v=20260628a~t 전부 배포 LIVE (최신 git tag `v20260628t`, 작업 트리 clean)**. 버전별 상세는 위 "2026-06-28 세션" 항목 + 루트 `CLAUDE.md` Version(a~t) 참조. **오늘 큰 줄기**: 인재양성 로드맵 아이콘·글자 키움 / CEO·문의하기 뱃지 정리(CEO=visibility:hidden 위치고정, 문의하기=삭제+위로) / why-pro "같은 시간 / 다른 결과" 블루 프레임(#solutions 동일)+우측 카드 순서 스왑(보장분석 AI 최상단, 본문만) / 승격 구조 파란 배경 단계별 순환(7s) / **STEP1 궤도 세련화**(빛 스윕+글래스 위성+중앙 PRO+코어 네이비+위성 블루통일) **+ 클릭 시 프로사업단총괄 교육섹션(#edu-horiz-scroll)으로 이동(히어로 플리커 제거)** / 헤더 네비 글자16·간격gap-12·지원하기 셰브론 제거 / 히어로 제목 순백색·크기 7xl→6xl·서브타이틀 font-normal / 문구·태그 다수("프로 솔루션"·"PRO AI 통합 시스템"·"통합 금융계산기"·"AI 시스템 체험하기"·stories 캡션·전문역량 태그 #DB영업/#소득극대화/#전문가양성 등) / 시스템현황 클릭 글로우 cyan→블루 / STEP2·3 배경 #05030F 통일·리스트 애니 시작잔상 수정 / CEO 서명 복구(assets/images/ceo-signature.png). **전부 DOM 실측·콘솔0.** ⚠️ **preview_eval=백그라운드 탭(docHidden)→CSS 애니 frozen**(screenshot 타임아웃·동적 색 dim) → `getAnimations().currentTime` 수동주입 또는 CSS 규칙 직접읽기로 검증할 것.
...
## 다음 대화 첫 질문 (복붙용)
"ai-branch(AI 홈페이지) 이어서 작업할게. `ai-branch/CLAUDE.md`랑 `ai-branch/.claude/memory.md` 읽어줘. 2026-06-28에 v=20260628a~t 전부 배포·검증 완료(최신 tag `v20260628t`) — 인재양성 로드맵·CEO/문의하기 뱃지·why-pro 블루프레임+카드순서·승격구조 순환·STEP1 궤도 세련화(중앙 PRO)+교육섹션 이동·헤더 네비·히어로 제목/서브타이틀·문구/태그 다수·시스템현황 글로우·STEP2/3 배경·CEO 서명 복구 등. 오늘은 [Web3Forms 키 교체(승인?) / 새 작업: ___] 중 ___ 부터 하자."
