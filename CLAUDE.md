...
## 프로사업단총괄 섹션 (v=20260407b — 진행 중)

### 현재 상태
- **오버레이 방식**: `#pro-intro-overlay` (ceo-overlay 패턴, `</body>` 직전)
- **Scene 1 (문 애니메이션)**: CSS + HTML 구현 완료, JS 스크롤 연동 수정 완료 (overlay.scrollTop 기반)
- **Scene 2 (지사 캐러셀)**: 14장 카드 자동 순환 (CSS @keyframes, 카드 복제 무한 루프)
- **워크스루 슬라이드쇼**: 성수 본점 6장 Ken Burns 효과 (각 4초, 24초 루프, 다른 줌/패닝 방향)

### 다음 세션 TODO (문 디자인 전면 교체)
- ❌ **현재 문 디자인(석조+나무)은 다크 테크 테마와 안 어울림** → 삭제 예정
- ✅ **"빛의 슬릿" 방식으로 교체 결정**: 가느다란 #27398c 빛 한 줄 → 점점 넓어지며 성수 본점 사진 드러남 → 화면 전체 삼키며 "신세계" 전환
- 문 통과 후 → 성수 본점 6장이 사방에서 펼쳐지는 "폭발" 리빌 → 그 아래 지사 캐러셀
- 폰트 크기 통일 완료: `clamp(1.6rem, 3.5vw, 2.8rem)`
...
- **교육 10장**: 아직 미사용 (교육 인프라 블록용)
- **성수 본점 6장**: 워크스루 슬라이드쇼로 사용 중
- **행사 2장, 성과 2장, 해외연수 1장, 영상 5개**: 아직 미사용
- 경로: `프로 사진/` + `프로 사진/프로사업단총괄 성수 본점/`

### 인라인 섹션 (숨김 처리)
- `#pro-carousel-section` (인라인, display:none) — 오버레이 안에 별도 캐러셀 있으므로 불필요

## Version
- **v=20260407b** — 프로사업단총괄 오버레이 (문+캐러셀+워크스루), 푸터 통일, 폰트 축소 (진행 중)
