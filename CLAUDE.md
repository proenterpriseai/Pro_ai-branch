...
- **v=20260710g~k** (**main 직커밋 LIVE**, tag `v20260710k`, main `3a8ab80`, 2026-07-10) — 대표님 피드백 3라운드. 상세 → auto-memory `session-20260710-aibranch-ceo-feedback.md`.
  ...
  - **l**: 대표님 정정("공식홈의 밝은 로열블루 실크를 원함" — 메인 히어로 씬은 어두운 스포트라이트 톤이라 다름) → **자체 raw WebGL fragment shader로 재현**(`pro-hero-silk-scene`, 의존성 0, 대각 주름 5중첩 warp+새틴 시닝, `smoothstep(-3.0,1.05)`). 오버레이 첫 오픈 lazy init + 닫히면 rAF 정지 + reduced-motion 정적 + CSS fallback glow. **튜닝법**: 페이지 내 오프스크린 하네스로 후보 렌더→프레임 base64 캡처 시각 대조+수치 지표(밝은면적 66%·평균색 rgb(62,93,193)≈공식홈) 수렴. ⚠️공식홈(proenterprise.co.kr) 히어로 = **아임웹 인터랙티브 배경(vendor ThreeCanvas.js)** — 빌더 내장이라 설정 추출 불가(정적 HTML에 색/프리셋 없음, 런타임 주입).
