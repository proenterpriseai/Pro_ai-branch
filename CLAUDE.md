...
- **v=20260918a** (**main 직커밋 LIVE**, tag `v20260918a`, 2026-09-18) — **푸터 이메일 추가**(전략실장 요청, A안=정보 블록 4번째 줄). `이메일 proenterprise@incarproent.com`(mailto 링크, 라벨 흰색+값 70% 톤 기존 위계 동일)을 등록번호 줄 아래에 추가 — **6곳 전부**(메인 footer + 오버레이 5: 문의하기 `#contact`/프로솔루션/CEO/성장시스템/조직소개). 마크업 전용, JS·CSS 무수정 → 트리플체크 A 비대상(v=20260721b 선례).
  ...
  - **검증(R1)**: 로컬 3098 PC 1280·모바일 375 양쪽 — 6곳 전수 기하 실측(mailto 링크 6개 정확, 정보 블록 4줄, 한 줄 렌더, 모바일 우측 여유 112px+), 가로 overflow 0, 콘솔 에러 0. (R2 라이브 실측은 배포 직후 수행 — 커밋 로그 참조.)
- **v=20260826a** (**main 직커밋 LIVE**, tag `v20260826a`, main `e29f003`, 2026-08-26) — **교육 패널4 "WM센터 주관 전문가 양성과정" 개편 + 교육 4패널 제목 y 통일**(전략실장 요청·문구 확정). 마크업+CSS 보정만, JS 무수정.
  - **패널4 개편**: 구 "경제/법인 전문가 과정"(펀드투자권유대행인/헥사곤/법인컨설팅 아카데미 3과정) → **과정 2개**: 재무설계 전문가 과정(자산관리 · 투자 컨설팅) / 법인컨설팅 전문가 과정(법인 절세 · 가업승계 컨설팅). 설명 "WM센터장이 직접 이끄는 개인 재무설계와 법인 컨설팅 전문 과정으로,<br>보험을 넘어 종합 자산관리 전문가로 도약하는 최상위 과정입니다." + 태그 #WM센터/#재무설계/#법인컨설팅. 이미지 4장 신규 `AI 홈페이지/WM센터/`(재무설계 포스터+커리큘럼=course 0 / 법인컨설팅 포스터+커리큘럼=course 1) — 기존 `data-course` 하이라이트 연동 JS 그대로 재사용. ⚠️제휴 아코디언(#partner-network)의 "헥사곤 투자자문" 카드는 별개=무수정.
  - **4패널 제목 y 통일**: `.edu-hp-text`가 세로 중앙정렬이라 과정 수(4/3/3/2)에 따라 제목 높이가 제각각(1440×900 기준 237/237/200/184) → 패널3(가장 김) 기준으로 **패널1·2 `edu-hp-text--3courses`(+74px)·패널4 `edu-hp-text--2courses`(+148px) 하단 패딩 보정**(중앙정렬이라 패딩의 절반만큼 상향, 콘텐츠 높이차=고정 px라 뷰포트 높이 무관). `@media(min-width:769px)` 한정=모바일 무영향.
  - **검증(R1·R2)**: 로컬+라이브 PC 1440×900(4패널 전부 200px)·1366×768(전부 134px·잘림0)·모바일 375(보정 미적용 32px·오버플로우0) / 이미지 4장 라이브 200·로드 확인 / 하이라이트 풀사이클 실측 / 라이브 HTML==로컬 byte IDENTICAL / 구 문구·구 이미지 참조 잔존 0 / 콘솔 0. docs/edu-horiz-scroll.md 매핑 표 동기화.
- **v=20260728e** (**main 직커밋 LIVE ✅전략실장 확인 완료**, tag `v20260728e`, main `43b3fab`, 2026-07-28) — **헤더 "AI 시스템" = 시연 오버레이 직행**(전략실장 요청: 체험까지 2클릭 → **1클릭**). 시연 진입은 **2갈래 유지** — ①헤더/모바일 메뉴 ②`#solutions` 섹션의 "체험하기" 버튼.
...
- **v=20260728b** (**main 직커밋 LIVE ✅전략실장 PC·모바일 양쪽 확인 완료**, tag `v20260728b`, main `34d1f47`, 2026-07-28) — **AI 시스템 시연 재진입 시 "최초 상태" 리셋 + 계산기 매칭 fix**(전략실장 요청: PC·모바일 모두 시연 후 PRO 로고/다른 헤더 오버레이를 다녀오면 이전 작업이 남아 있고 이전 시연이 계속 작동). **전략실장 정의: "최초 상태" = 인사말("안녕하세요! …시스템 현황에서 해당 AI 시스템을 클릭 후 사용해보세요.")이 타이핑되는 화면 → 끝나면 우측 시스템 현황에서 시연 대상 선택**. 트리플체크 A 3라운드(1차 NO-GO 블로커 2건 → 수정 후 GO → 델타 GO). 상세 → auto-memory `session-20260728-aibranch-demo-reset.md`.
  ...
  - **④ 🐛 계산기 카드 매칭 fix(한 달 잠복)**: `systemNameMap` 키 `'통합금융계산기'` vs DOM 라벨 `'통합 금융계산기'` 불일치 → `sysKey='all'`로 떨어져 **9(실제 11)모듈 패널이 2026-06-28(v=20260628g 라벨 띄어쓰기 변경)부터 열리지 않음**. 키 정정 + **`_solResolveSysKey`(정확일치 → 공백무시 → 'all') 폴백**으로 재발 방지. ⚠️교훈: **라벨 문구를 바꿀 때 그 라벨을 키로 쓰는 map을 반드시 동반 확인**.
  - **⑤ ④가 드러낸 연쇄 버그**: 계산기 모듈 모달(`#sol-calc-modal`, **body 직속 z-index 100000**)을 열어둔 채 오버레이를 닫으면 **메인 화면 위에 잔존** → 리셋에 `_solCalcCloseModal()` 추가(×·ESC·배경클릭이 쓰는 그 함수라 무충돌).
  - **⑥ DB영업관리 입력값**: `DBSALES_STATE`가 모듈 레벨이라 닫아도 생존 → 초기 스냅샷(JSON) 복원. **세션 내(오버레이 유지) 다른 시스템 갔다 와도 값은 유지**(시연 중 소실 방지).
  - **⑦ 인사말 타이핑 선재 버그 2건**: `run()`이 `#sol-main-greeting`을 **매 실행 재조회**(리셋이 innerHTML을 갈아 끼우면 기존 참조가 detached) + **350ms 지연 핸들 `pending` 추적**(열기→닫기→열기 연타 시 setInterval 2개가 경쟁해 상대 핸들을 clear → 인사말이 중간에서 영구 정지).
  - **검증**: 로컬 3098 DOM 실측 — PC·모바일 375 왕복 리셋 / 7개 시스템 매칭 전수(placeholder·전용패널·+버튼) / 계산기 11모듈 런타임 스윕(결과 렌더·서브탭·키보드 가드·CTA) / 스트리밍·PDF 이탈 시 API 호출 **0회** 계측 / F5 복원 y2500→2537·`manual` 회귀 0 / 콘솔 에러 0. ⚠️**미리보기 탭이 hidden이면 rAF·scroll 이벤트·CSS 애니가 정지/스로틀**되어 focus·타이핑 속도 계측이 무의미 → 합성 이벤트·`setInterval` 래핑 카운트로 대체(v=20260628c 교훈과 동일).
  - **의도적 미변경(전략실장 결정)**: 계산기 **"9개 모듈" 문구 유지**(실제 11개 — 패널 부제·데모 배지·인사말·잠금 CTA·쇼케이스 우측 패널 등 6곳 드리프트, 인사말은 `통합금융계산기` 붙여쓰기로 헤더와 표기도 불일치). **후속 이월**: W-3(스크롤 경계에서 검색어 clear — 자기복구·차단 아님) / `_solCalcBindPanelEvents`의 `#sol-calc-cta-btn` 죽은 조회(v=20260518j 잔재) / `systemNameMap` 상속키(`toString` 등) 방어는 `Object.create(null)` 권장.
...
- **v=20260717d** (**main 직커밋 LIVE**, tag `v20260717d`, main `eb24f64`, 2026-07-17) — **오버레이 우측 상단 X 닫기 버튼 전체 제거**(사용자 요청: 대부분 네비로 이동 → X 시각적 중복·거슬림). PC·모바일 공통(마크업 삭제라 뷰포트 무관). 상세 진실원 → auto-memory `session-20260717-aibranch-header-footer-polish.md`.
  ...
  - **닫기 경로 유지**: `closeAllOverlays()`(9678)가 active클래스·aria-hidden·body클래스·`body.style.overflow=''` 복원 전부 처리 → 네비(로고 `data-nav=top`·메뉴) 클릭이 navigateTo→closeAllOverlays 경로. CEO는 추가로 ESC(10898)·배경 클릭(10902)·popstate 유지. 고정 네비 바 PC/모바일 상시 노출 → 닫는 길 항상 존재.
  - **검증**: 375·1280px DOM 실측 `button.ceo-overlay__close` 0개, 각 오버레이 열기·네비 닫기+body 스크롤 복원(`hidden auto`)·CEO 가드후 열기+ESC 닫기 정상, 콘솔0. 라이브(`ai-branch.vercel.app?cb=`) v마커5·X버튼0 확인. 남은 CSS `.ceo-overlay__close` 규칙 5곳(838·858·2713모바일·7752·7753)은 대상 없는 죽은코드·무해로 존치. ⚠️교훈: **공유 클래스 엘리먼트 삭제 전 무가드 `querySelector→addEventListener` 여부 확인**(IIFE 전체 사망 방지).
- **v=20260717c** (**main 직커밋 LIVE**, tag `v20260717c`, main `6364bf5`, 2026-07-17) — 성장시스템(talent) 오버레이 모바일 폴리시 2건. 상세 → auto-memory `session-20260717-aibranch-header-footer-polish.md`.
  - **`.tsk-num`(01~04 카드 배경 숫자) 모바일 숨김**(`@media(max-width:768px){display:none}`) — 좁은 카드 둥근모서리(`overflow:hidden`+`border-radius:30px`)에 잘려 보이던 것 해소, PC 유지.
  - **GROWTH 인트로 문단 PC/모바일 분리**(01카드 `.tsk-copy .body`, ~15359): PC용 `<p class="body hidden md:block">`(기존 `<br>`) + 모바일용 `<p class="body md:hidden">`(신규, "WM 전문가 과정" 띄어쓰기 + 절 단위 4줄 `<br class="br-keep">`×3). ⚠️PC↔모바일 문구 divergence(띄어쓰기)=사용자 "모바일만" 명시 의도.
- **v=20260717b** (**main 직커밋 LIVE**, tag `v20260717b`, 2026-07-17) — 모바일 푸터 줄바꿈 정리 + WM 배지 삭제.
  - 모바일 푸터 **TEL 번호(0507-1379-0640) `white-space:nowrap`** 한 줄 고정(메인1+오버레이5) / **COPYRIGHT `font-size:clamp(7.5px,2.3vw,13px)`+nowrap** 한 줄(320~414px 잘림0, 데스크톱 13px) / **WM센터 "2026년 7월 출범" 배지(`.wm-badge`) 삭제**(PC·모바일 공통, 15936줄 span 제거).
- **v=20260717a** (**main 직커밋 LIVE**, tag `v20260717a`, main `b5b20e0`, 2026-07-17) — 헤더/푸터 폴리시 5건(전부 공식홈 proenterprise.co.kr 기준 정합).
  - **헤더 로고**: 세로기준 `h-[30px]` → **가로기준 `w-[114px] md:w-[122px] h-auto`**(PC122/모바일114px). 이유=로고 절대크기는 공식홈과 동일했으나 ai-branch 유리 알약(64px)이 공식홈 헤더(70px)보다 낮아 비중 47% vs 43% → 알약 내 비중을 공식홈 43%로 맞춤. (`a[data-nav="top"] img`.)
  - **푸터 6곳 로고 삭제 + 회사명 상단 이동**(메인1+오버레이5): 로고 `<img>` 제거 → TEL 왼쪽 같은 줄에 "인카금융서비스 프로사업단총괄" `<p>`(메인 `text-lg font-bold` / 오버레이 `18px/700`), 정보블록은 주소/총괄대표/등록번호만. **정보 라벨 순백색**(`text-white`/`#fff`, 값은 0.7톤 유지=위계 보존). **COPYRIGHT 10→14px**. **헤더 메뉴 굵기 600→700**(PC·모바일, 공식홈 `bold/16px` 동일).
- **v=20260714b** (**main 직커밋 LIVE**, tag `v20260714b`, main `9a8f66c`, 2026-07-14) — 모바일 폴리시 5건(배너 크롭·행간·캐러셀 부제·Workshop poster·캡션 br) + **🐛헤더 색 고착 fix**(4개 오버레이 전부 옵저버+scroll 리스너, `window.__applyOverlayNav` 노출). 데스크톱 회귀0. 상세 → auto-memory `session-20260714-aibranch-mobile-polish.md`.
- **v=20260713h** (**main 직커밋 LIVE**, tag `v20260713h`, 2026-07-13) — 오버레이 X 버튼 모바일 겹침 fix(사용자 세션). **(⚠️ v=20260717d에서 X 버튼 자체가 제거되어 이 fix는 무의미화됨.)** 모바일에서 `.ceo-overlay__close`(fixed top:24/right:24, 48px)가 네비 알약 우측 끝 햄버거(294~334px)와 거의 완전 겹침(303~351px, X z-index가 높아 메뉴 탭이 X로 먹힘) → 모바일 미디어쿼리로 **top:92px(알약 하단 82px 아래)·right:0.75rem·42px** 이동. CEO/조직소개/성장시스템 3오버레이+레거시 #contact 공통 적용, 프로솔루션은 원래 X 없음(배경 탭 닫기). 데스크톱 원위치(24px·48px) 회귀0 실측.
