...
## 2026-07-04 세션 — Nexora 리디자인 (브랜치 `preview/nexora-restyle` `50605e4`, 🔴 main 미머지 · 전략실장 프리뷰 검증 중)

> 전략실장이 Nexora 레퍼런스(HTML 제공)를 보고 "노출 없이 프리뷰로 확인→최종 결정" 방식 합의. 피드백 4라운드 반영 완료. 상세 스펙은 루트 CLAUDE.md `v=20260704a~d` 항목.

### 작업 요약 (4커밋: d50cd52→45da4f7→27bcac6→50605e4)
1. **7대 쇼케이스 SaaS 앱화**: 크롬 제거→앱 헤더("AI 통합 시스템")+실동작 검색창(시스템 자동 전환)+KPI 4카드(전부 동적)+우측 패널 계정 칩 행(68px 라인 일치)·엔진 정보 KPI행 정렬. "AI 시스템 체험하기" 버튼 제거.
2. **인재양성 스택 카드**: 섹션 1~3 → 01~04 스티키 스택(목업 4종 전부 동적 루프). 파트너 아코디언·WM센터 무손상, 문구 3건 변경(전략실장 확정).

### 🔴 다음 세션 할 일
1. **실사진 아바타**: 전략실장이 사진을 `assets/images/consultant-avatar.jpg`로 저장(채팅 붙여넣기 이미지는 파일 저장 불가) → git add+커밋 → 자동 적용(onerror P 폴백 이미 구현). object-position `center 16%` 얼굴 크롭 미세조정 여지.
2. 전략실장 추가 피드백 반영 반복 → **최종 OK 시 main FF 머지**(+구 roadmap-* 데드코드 정리 검토).

### ⚠️ 이 세션 신규 교훈 (영구)
- **오버레이 내 sticky stuck 위치 ≠ CSS top값**(스크롤포트 원점 오프셋, 실측 108px vs top:88px) → 스택/패럴랙스 JS는 고정 임계값 대신 **요소 간 실거리(gap) 기반**으로.
- **IO(IntersectionObserver)는 백그라운드 탭에서 동결** → 뷰포트 진입 토글은 스크롤 핸들러 rect 실측이 검증 가능하고 견고. rAF도 동일(→ setTimeout).
- 스택 카드 완전 은폐 = top 통일 + **열림 시점 JS 높이 균등화** 세트(카드 높이 제각각이면 꼬리 노출).
...
### ✅ 배포 내용 (전부 인재양성/프로사업단총괄 오버레이 — 700명 라이브)
1. **WM센터 섹션 신규**(`#wm-center`): 프로사업단총괄 오버레이 교육↔조직문화 사이. "2026년 7월 출범"(coming-soon, 실제 7월 중순 출범 예정) + 2트랙(법인/CEO·개인/고액자산가, **역할만**) + 혜택 3카드. 🔴 **내부문서(수수료 70/30·출장비 15/30만·센터장 번호 이기수/우태용·신청서) 전부 공개 제외** — 영입 페이지엔 혜택 내러티브만.
2. **교육 가로스크롤 공백 축소**: 와이드 모니터 가운데 ~630→55px(1180px 밴드).
3. **성과 카드 ↑ 화살표** 블루→흰색.
4. **EXPERTISE 재설계**(`#talent-expertise`): 자격 사다리(보험설계사→AFPK→CFP→WM) + 증거(`pro보험스쿨`/`CFP 2·AFPK 5`/직강 `20명` — **대표 제공 실수치**) + 소득 한 줄. 컴플라이언스: "부동산·세무 가능한 컨설턴트"(단정) → "종합 설계 역량"(자격 기반 톤).
5. **제휴 네트워크 아코디언 신규**(`#partner-network`): 5사 **헥사곤→모기지→대진→글로비→보케어**(헥사곤 기본 열림). 이미지=상단 가로 배너(cover). 🔴 **글로비·대진 스크린샷 텍스트가 우측~59% 박혀**→캔버스 좌측 18~19% 크롭+JPEG(globee 3.4MB→140KB, daejin 2MB→82KB). 5사 **수익률(헥사곤 20%대)·수수료(글로비 50%·대진 1~20%) 전부 공개 제외**(투자광고·내부정산 리스크), 보케어 무상제공만 강조.
6. **메인 #branch-map 체크마크** 씰(badge)+흰체크로 교체.

### ⚠️ 기술 교훈 (중요)
- **헤드리스 프리뷰(preview_eval) 한계**: ① `transition:flex`(단축) 동적 변경 시 `0 0 0%`로 멈춤 → **`flex-grow`만 트랜지션**으로 회피. ② flex-grow 동적 변경 시 width 재레이아웃 미갱신(transition:none이면 정상=실브라우저 OK). ③ Three.js rAF로 **스크린샷 타임아웃** → **DOM 실측**으로 검증(전 작업 콘솔0).
- **채팅 붙여넣기 이미지는 파일 저장 불가** → 사용자가 repo `assets/images/partners/`에 직접 저장(한글·공백 파일명→ASCII 리네임. `assets/`는 `.gitignore`의 `!/assets/`로 추적됨).
- **Vercel 프리뷰 배포 = SSO 보호(302)**, 프로덕션(ai-branch.vercel.app)은 공개. 프리뷰 URL은 `vercel ls ai-branch`로 획득.
- 스크린샷 이미지를 가로 배너에 넣을 때 텍스트 위치는 CSS로 못 옮김 → **소스 크롭만이 답**(캔버스 toDataURL→Node base64 디코드 저장).

### 🔴 남은 일(보류)
- **모기지·헥사곤·보케어 PNG 압축**(~6MB → 수백KB, globee/daejin처럼 캔버스 리사이즈+jpg). 무거움(오버레이+lazy라 치명적이진 않음).
- **배너 크롭% 미세조정** 여지(이미지별 object-position / 글로비·대진 크롭 비율).
- **Web3Forms 키 교체** 여전히 보류(대표 승인 대기, 현재 키 `c96794c6…`=전략실장 수신).
- (참고·미래) `api/chat.js` `gemini-2.5-flash` 2026-10-16 퇴역 → 그 전 교체.
...
### ✅ v=20260628g — 시스템 현황 활성 글로우 블루화 + 통합 금융계산기 + STEP2·3 배경 통일 (배포 LIVE)
...
> ✅ 2026-06-29: **v=20260629a 프로덕션 배포 LIVE 700명 (tag `v20260629a`, main `17966f8`, 작업 트리 clean, `preview/wm-center` 12커밋 FF 머지)**. 상세=위 "2026-06-29 세션" + 루트 `CLAUDE.md` Version(v=20260629a). **큰 줄기**: WM센터 섹션 신규(프로사업단총괄 오버레이, 2026년 7월 출범 배지, 내부문서 전부 제외) / 교육 가로스크롤 가운데 공백 축소(1180 밴드) / 성과 화살표 흰색 / 인재양성 EXPERTISE 재설계(자격 사다리+증거 pro보험스쿨·CFP2·AFPK5·직강20명) / **제휴 네트워크 아코디언 신규**(헥사곤→모기지→대진→글로비→보케어, 이미지 상단 배너, 글로비·대진 크롭+압축) / 현황 체크마크 씰 / "PRO 네트워크". **전부 DOM 실측·콘솔0.** ⚠️ **헤드리스 한계**: `transition:flex` 동적 멈춤→`flex-grow`만 트랜지션, flex-grow width 미갱신(실브라우저 OK), 스크린샷 타임아웃→DOM 실측.

1. 🔴 **(보류) 모기지·헥사곤·보케어 PNG 압축** — ~6MB 무거움. globee/daejin처럼 캔버스 리사이즈+jpg로 수백KB화. (오버레이+lazy라 치명적이진 않음.)
2. 🔴 **(보류) Web3Forms 키 교체** — 대표 승인 후. 현재 키 `c96794c6…`=전략실장 수신. 승인 시 `proenterprise@incarproent.com` 새 키 발급→`_submitContactForm` 1줄 교체.
3. (선택) WM센터·제휴 네트워크 등 라이브 육안 모니터링(ai-branch.vercel.app). 배너 크롭%·object-position 미세조정 여지.
4. (참고·미래) `api/chat.js` `gemini-2.5-flash` **2026-10-16 퇴역** → 그 전 교체.
